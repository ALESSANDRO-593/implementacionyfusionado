...
export type TipoVehiculoParqueadero = 'AUTO' | 'MOTO';
export type ModalidadParqueadero = 'DIARIO' | 'MENSUAL';
export type EstadoTicketParqueadero = 'PENDIENTE' | 'APROBADO' | 'RECHAZADO' | 'ANULADO';

export interface VehiculoParqueadero {
  id: number;
  placa: string;
...
export interface TicketParqueadero {
  ...
  monto: number;
  estado: EstadoTicketParqueadero;
  fechaSolicitud: string;
...
export interface DescargaTicketParqueadero {
  nombreArchivo: string;
  contenidoBase64: string;
}
export interface EstadoParqueaderoUsuario {
  vehiculo: VehiculoParqueadero | null;
  ticket: TicketParqueadero | null;
}
