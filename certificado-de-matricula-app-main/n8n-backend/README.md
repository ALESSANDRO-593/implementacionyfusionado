...
- **`workflows/`** — los 11 workflows de n8n importables:
  ...
  | `workflow-consultar-tickets.json` | 6.1 | Estado de los tickets del estudiante identificado |
  | `workflow-resetear-contrasena-correo.json` | 6.2 | Reseteo automático — implementado, no expuesto en el menú actual de la app |
  ...
  4.1, 6, 6.1, 9) exigen una sesión OTP verificada en los últimos 20 min —
  ver "Autenticación" en `CONTRATO-API.md`.

  **Nota**: `consultar-tickets` se retiró temporalmente durante el
  desarrollo y se reincorporó al integrar con el servidor institucional
  compartido — usa la misma tabla `tickets` que el panel administrativo del
  equipo web, así que ambas apps ven siempre el mismo estado.
...
2. Abrir n8n → `Workflows` → `Import from File` (o arrastrar cada `.json`)
   para los 11 archivos de `workflows/`.
...
7. En `workflow-resetear-contrasena-correo.json`, configurar la credencial
   ...
   esté disponible.
8. Activar los 11 workflows y confirmar que la URL base coincide con
...
- **Idempotencia**: el workflow busca antes de crear (para responder con el
  certificado existente en vez de un error). En el entorno de desarrollo
  local (`init.sql`) esto además está forzado con un constraint
  `UNIQUE(estudiante_id, periodo_lectivo_codigo)` en `certificados`. La
  base de datos real del instituto **no tiene ese constraint** — se
  confirmó al integrar en producción que ya existían certificados
  duplicados de pruebas previas del equipo web; agregarlo ahí requeriría
  limpiar esos datos primero, coordinado con el otro equipo (ver
  `ESQUEMA-BD.md`).
