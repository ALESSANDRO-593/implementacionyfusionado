...
workflows/
  ...
  workflow-consultar-tickets.json              # dominio: trámites/tickets (estado, estudiante)
  ...
  workflow-resetear-contrasena-correo.json     # dominio: trámites (especializado, automático, estudiante — no expuesto en el menú actual)
...
**Nota**: `consultar-tickets` (seguimiento de estado) se implementó, se
retiró temporalmente durante el desarrollo, y se **reincorporó** al
integrar con el servidor institucional compartido en agosto de 2026 — usa
la misma tabla `tickets` que el panel administrativo del equipo web, sin
sincronización aparte. El cierre automático de tickets vía respuesta de
Gmail (`detectar-respuesta-ticket`) sí se descartó de forma definitiva: ese
cierre lo hace el personal administrativo desde el panel web. Ver "Roadmap
de fases" (Fase 1 y Fase 7) para el detalle histórico completo.
...
- **Autenticación del webhook — ✅ implementado.** Los 11 workflows con
  ...
  lee la clave de `environment.apiKey`.
- **Credenciales de Postgres**: nunca hardcodear usuario/contraseña en los
  nodos — usar el sistema de credenciales de n8n.
...
- **Sobre la `X-Api-Key`**: en producción (VPS institucional compartido)
  esta clave queda intencionalmente versionada en
  `environment.ts`/`environment.prod.ts`, para que cualquier equipo que
  continúe el desarrollo pueda clonar el repositorio y ejecutar la app sin
  pasos adicionales de configuración. No sustituye a la sesión OTP como
  control de acceso a datos personales (ver el punto siguiente) — es solo
  el primer filtro contra tráfico que no pasa por la app.
...
Todo lo descrito en `ARRANQUE-LOCAL.md` (Docker local, `init.sql`, datos de
prueba) sigue siendo válido como entorno de **desarrollo**. Desde agosto de
2026 la app además corre en **producción real**, integrada en el mismo VPS
institucional y la misma instancia de n8n que usa la aplicación web
construida por el otro equipo del proyecto de titulación — no un servidor
aparte. Detalle completo (arquitectura del servidor, nginx, empaquetado
como APK, incidentes resueltos) en `Manual-Tecnico-YaviBot.html`, sección
11 "Despliegue en producción". Resumen de lo que cambió respecto al plan
original de este documento:

1. Los workflows se importaron a la instancia de n8n ya existente en el
   VPS (compartida con el equipo web), no a una instancia nueva — la
   credencial Postgres apunta directo a la base de datos real del
   instituto (~1400 estudiantes, ~107 docentes).
2. **Los constraints previstos no se "aplicaron" tal cual se habían
   planeado — se descubrió que el esquema real ya tenía restricciones
   propias, distintas a las asumidas**: `tickets.codigo` y
   `alertas.codigo` son `NOT NULL` **y** `UNIQUE` en producción (no
   `nullable` como se planeaba aquí). Los workflows se ajustaron para
   insertar un valor temporal aleatorio y reemplazarlo por el código
   correlativo definitivo justo después del insert, en vez de dejar el
   campo vacío — ver `CONTRATO-API.md` §4 y §6.
3. Los `usuarios_panel.correo` de prueba ("Juan Pérez", etc.) **ya no
   existen** — el responsable de cada tipo de trámite se resuelve en
   producción vía `rol_tipos_solicitud` (no `asignaciones_responsables`,
   que no existe en el esquema real) contra contactos reales del
   instituto (ej. Responsable de Laboratorios: Carlos Ruiz / Viviana
   Cuatin).
4. El volumen de fotos de incidencias vive en `/root/yavibot/storage` del
   VPS, montado en el contenedor de n8n en `/data/storage` — la misma
   carpeta que ya usaba el sistema del equipo web para sus propios
   adjuntos, sin necesidad de una variable `N8N_RESTRICT_FILE_ACCESS_TO`
   adicional en esa instancia.
5. **Hecho** — la URL de verificación del QR (`workflow-generar-certificado.json`,
   nodos "Formatear Respuesta" y "Formatear Respuesta Existente") ya no
   apunta a `localhost`: apunta a la app publicada en el propio VPS
   (`/app/verificar-certificado/{codigo}`), y `environment.prod.ts`
   apunta al backend real del VPS.
6. **Pendiente** — rate-limiting por IP en el proxy del VPS para
   `/consultar-estudiante` (el único endpoint sin OTP previo, solo
   CAPTCHA): sigue sin implementarse, mismo razonamiento que antes (es
   responsabilidad del proxy, no de n8n/Postgres). Ahora que el VPS y el
   dominio sí existen, ya no hay razón para seguir postergándolo.
...
Hay cinco trámites/flujos implementados a nivel de workflow; cuatro están
expuestos en el menú actual de la app:

| Trámite | Rol | Automatizado | Genera documento | En el menú |
|---|---|---|---|---|
| Certificado de Matrícula | Estudiante | ✅ Sí (workflow especializado) | ✅ Sí (QR + correo, PDF opcional) | ✅ Sí |
| Anulación de Matrícula | Estudiante | ❌ No (requiere revisión) | ❌ No | ✅ Sí |
| Consultar estado de tickets | Estudiante | ✅ Sí (solo lectura) | ❌ No | ✅ Sí |
| Reseteo de Contraseña de Correo Institucional | Estudiante | ✅ Sí (workflow especializado) | ❌ No | ❌ No — ver nota abajo |
| Reportar incidencia de laboratorio | Docente | ❌ No (requiere revisión) | ❌ No (foto opcional adjunta) | ✅ Sí |
...
**Nota sobre el menú**: el Reseteo de Contraseña se retiró del menú visible
de la app en la fase de cierre del proyecto — el workflow
(`POST /resetear-contrasena-correo`) sigue activo en el servidor con su
validación, cooldown y auditoría probados (ver estado de la credencial de
Google Workspace más abajo), y el código de la app (`chat.page.ts`,
`estudiante.service.ts`) permanece en el repositorio para reactivarlo con
un solo cambio si se decide reincorporarlo.
...
4. La app (`chat.page.ts`) guarda el `Usuario` identificado (unión
   ...
   `tipoUsuario`: `OPCIONES_MENU_ESTUDIANTE` (Certificado de Matrícula,
   Anulación de Matrícula, Consultar estado de tickets — el Reseteo de
   Contraseña está implementado pero no aparece en este menú, ver más
   abajo) u `OPCIONES_MENU_DOCENTE` (Reportar incidencia en laboratorio +
...
responsable vía `rol_tipos_solicitud` (tipo `ALERTA_LAB`) y le envía
...
1. **Fase 1 (hecha)**: consultar-estudiante, enviar/verificar-ticket,
   ...
   Matrícula (esta fase también incluyó `consultar-tickets`, retirado
   temporalmente y luego reincorporado — ver Fase 7). Esquema temporal de
   base de datos (ya reemplazado, ver fase 3).
...
7. **Fase 7 (revertida y luego reincorporada)**: se había construido el
   cierre automático de tickets vía respuesta de Gmail
   (`detectar-respuesta-ticket`) y el endpoint `consultar-tickets` para
   que el estudiante viera el estado de sus trámites. Ambos se
   **eliminaron** por indicación de la tutora durante el desarrollo — el
   seguimiento de tickets lo cubre el sistema web. Al integrar con ese
   sistema en producción (Fase 10), se confirmó que ambas apps comparten
   la misma tabla `tickets` sin duplicación posible, y `consultar-tickets`
   se **reincorporó** a la app; el cierre automático vía Gmail sigue
   descartado de forma definitiva (lo hace el personal administrativo
   desde el panel web).
8. **Fase 8 (hecha)**: verificación pública de certificado por QR
   ...
   los endpoints que actúan por un estudiante/docente específico, límite
   de 5 intentos/10min al verificar el ticket, cooldown de 60s al
   reenviarlo, y cédula enmascarada en la verificación pública.
9. **Fase 9 (hecha, decisión de alcance)**: Reseteo de Contraseña de
   Correo Institucional se retiró del menú visible de la app en la fase de
   cierre del proyecto. El workflow permanece implementado y activo en el
   servidor — ver "Reseteo de contraseña: automático, no un ticket" arriba.
10. **Fase 10 (hecha)**: despliegue en el VPS institucional compartido con
    la aplicación web del proyecto, sobre la base de datos real (~1400
    estudiantes, ~107 docentes); adaptación de los workflows a diferencias
    puntuales del esquema real (`rol_tipos_solicitud`, restricciones
    `NOT NULL`/`UNIQUE` en `tickets.codigo`/`alertas.codigo`); corrección
    de la URL de verificación del QR; empaquetado de la app como APK
    (Capacitor + Android Studio); resolución de un incidente de
    corrupción de la base de datos de n8n durante el despliegue, sin
    pérdida de datos; respaldo completo del sistema antes de compartir
    acceso al servidor con equipos adicionales. Detalle completo en
    `Manual-Tecnico-YaviBot.html`, sección 11.
11. **Pendiente**: sub-workflows reutilizables (`sub-validar-estudiante`),
    error workflow global en n8n, rate-limiting por IP en el proxy del
    VPS para `/consultar-estudiante` (ver "Despliegue en producción"
    arriba).
