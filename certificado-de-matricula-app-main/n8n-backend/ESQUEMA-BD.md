...
La base de datos real del instituto tiene **~25 tablas** para un sistema
mucho más grande que solo certificados. Esta app (chatbot YaviBot) solo
lee/escribe en un subconjunto:
...
| `rol_tipos_solicitud` | ✅ (opcional) | qué rol del panel atiende cada tipo de trámite — resuelve el responsable a asignar |
...
7. **La idempotencia del certificado ahora SÍ está forzada por la base de
   ...
   duplicados) antes de aplicarse.

   **Confirmado en producción (agosto 2026)**: al integrar contra la base
   de datos real y compartida del instituto se verificó que, en efecto,
   **no tiene** el constraint `UNIQUE(estudiante_id, periodo_lectivo_codigo)`
   — y ya existían certificados duplicados reales, generados por pruebas
   del propio equipo web antes de esta integración. Agregar el constraint
   ahí requeriría primero limpiar esos duplicados históricos, es una
   operación sobre datos reales compartidos con otro equipo, y **no se
   hizo** sin coordinarlo explícitamente — queda pendiente como mejora,
   no como bloqueante: el workflow de esta app sigue haciendo el chequeo
   "¿ya existe?" antes de insertar, que es lo que evita que **esta app**
   genere duplicados nuevos.
...
> **Corrección confirmada contra la base de datos real del instituto
> (agosto 2026)**: esta tabla reemplaza a `asignaciones_responsables`, que
> **no existe** en el esquema real — era un supuesto de diseño de antes de
> integrar con la base de datos institucional. El propósito es el mismo
> (resolver a quién notificar/asignar un trámite nuevo), pero la relación
> es **por rol**, no por usuario individual: mapea qué `rol_id` puede
> atender cada `tipo_solicitud_id`. El responsable final se resuelve
> cruzando esta tabla con `usuarios_panel` (`rol_id` + `activo = true`).

```sql
rol_id (FK roles), tipo_solicitud_id (FK tipos_solicitud)
```
...
`rol_tipos_solicitud` (tipo `ALERTA_LAB`, sin filtro de carrera — en la
base real, los responsables confirmados son Carlos Ruiz y Viviana Cuatin).
A diferencia de Anulación de Matrícula, no hay límite de incidencias
