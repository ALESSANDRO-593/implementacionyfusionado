...
**Nota sobre la numeración**: el workflow interno `detectar-respuesta-ticket`
(cierre automático vía respuesta de Gmail) se eliminó por indicación de la
tutora — el cierre de tickets lo hace el personal administrativo desde el
panel del equipo web, no por respuesta de correo. El endpoint 5 se deja
libre en vez de renumerar todo el documento. `/consultar-tickets` (historial
de tickets del estudiante) sí se mantiene — ver §6.1.
...
`rol_tipos_solicitud`) y registra los eventos
...
`rol_tipos_solicitud`), el workflow busca su correo real en
...
del ticket ni la respuesta al webhook. Confirmado contra la base de datos
real del instituto: el correo notificado ya es el de la persona real
asignada a ese tipo de trámite, no un dato de prueba.
...
## 6.1 Consultar estado de tickets

**POST** `/consultar-tickets`

Devuelve el historial de tickets (`Anulación de Matrícula` y cualquier otro
tipo de trámite) del estudiante identificado, en el mismo estado que ve el
personal administrativo desde el panel del equipo web — misma tabla
`tickets`, sin sincronización aparte.
  ...
  "tickets": [
    {
      ...
      "tipo": "Anulación de Matrícula",
      "estado": "EN_PROCESO",
      "fechaSolicitud": "09 de agosto de 2026"
    }
  ]
}
```

`estado` viene traducido del vocabulario real de la base de datos
(`Pendiente` / `En Proceso` → `EN_PROCESO`, `Resuelto` → `COMPLETADO`) al
contrato que espera la app. Lista vacía (`{ "tickets": [] }`) si el
estudiante no tiene tickets registrados — nunca un error.
...
**Requiere sesión OTP reciente**, mismo patrón que el resto de endpoints que
actúan por una cédula específica.

---

## 6.2 Resetear contraseña de correo institucional (automático)
...
> **Estado actual**: el workflow está implementado, probado y activo en el
> servidor, pero la opción se retiró del menú visible de la app en la fase
> de cierre del proyecto. Este contrato se mantiene documentado por si se
> decide reincorporarla.
...
`certificados`, `qr_codigos`, `rol_tipos_solicitud`, `eventos`) y cómo
se relacionan.
...
- El responsable se resuelve vía `rol_tipos_solicitud` (tipo
